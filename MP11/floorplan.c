...
int is_internal_node(node_t* ptr) {
  ...
  // If node has any children, return true
  if (ptr->left != NULL || ptr->right != NULL) {
    return 1;
  }
  // Otherwise, return false
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  // If node b is equal to node a, return true
  if (b == a) {
    return 1;
  }
  // If node b doesn't exist (null) return false
  if (b == NULL) {
    return 0;
  }
  // Otherwise, recurse with parameters a and b's parent, effectively moving up the tree
...
void rotate(node_t* ptr) {
  ...
  // Create temporary copy of node's width
  int temp = ptr->module->w;
  // Swap the width and height, using temp to not lose track of width
...
void recut(node_t* ptr) {
  ...
  // If node's cutline is vertical, change it to horizontal
  if (ptr->cutline == V) {
    ptr->cutline = H;
  }
  // Otherwise, change it to horizontal
  else {
    ptr->cutline = V;
  }
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // Create temporary copy of node A's module pointer
  module_t* temp = a->module;
  // Swap the nodes' modules, using the temp to not lose track of a's module
  a->module = b->module;
  b->module = temp;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // Initialize 2 temp nodes, one for swapping parent's children and the other for swapping a and b's parents
  node_t* temp;
  node_t* temp_parent;

  // If A is a right node...
  if (a->parent->right == a) {
    // If B is a right node, swap A's parent's right ptr and B's parent's right ptr
    if (b->parent->right == b) {
      temp = a->parent->right;
      a->parent->right = b->parent->right;
      b->parent->right = temp;
    }
    // If B is a left node, swap A's parent's right ptr and B's parent's left ptr
    else {
      temp = a->parent->right;
  ...
  // If A is a left node...
    ...
    // If B is a right node, swap A's parent's left ptr and B's parent's right ptr
    if (b->parent->right == b) {
      temp = a->parent->left;
      a->parent->left = b->parent->right;
      b->parent->right = temp;
    }
    // If B is a left node, swap A's parent's left ptr and B's parent's left ptr
    else {
      temp = a->parent->left;
  ...
  // Set temp_parent to A's parent
  temp_parent = a->parent;
  // Swap A and B's parents, using temp_parent to not lose A's parent
  a->parent = b->parent;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // If node has a left child, recurse into it
  if (ptr->left != NULL) {
    postfix_traversal(ptr->left, nth, expression);
  }
  // If node has a right child, recirse into it
  if (ptr->right != NULL) {
    postfix_traversal(ptr->right, nth, expression);
  }

  // Allocate memory for new expression unit
  expression_unit_t * cur_e_unit = (expression_unit_t*)malloc(sizeof(expression_unit_t));
  // Set new expression unit's values equal to its respective node's values
  cur_e_unit->module = ptr->module;
  cur_e_unit->cutline = ptr->cutline;

  // Set pointer in array to a pointer to new expression unit
  expression[*nth] = *cur_e_unit;
  free(cur_e_unit);
  // Increment value at nth by 1
  *nth += 1;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // If function is at end of tree...
  if (n + 1 == num_modules) {

    // Allocate memory for new left and right nodes
    node_t* cur_left_node = (node_t*)malloc(sizeof(node_t));
    node_t* cur_right_node = (node_t*)malloc(sizeof(node_t));

    // Set up all values for new left node (no children, parent = par, no cutline, and correct module)
    cur_left_node->left = NULL;
    cur_left_node->right = NULL;
    cur_left_node->parent = par;
    cur_left_node->module = modules + n;
    cur_left_node->cutline = UNDEFINED_CUTLINE;
    par->left = cur_left_node;

    // Set up all values for new right node (no children, parent = par, no cutline, and correct module)
    cur_right_node->left = NULL;
    cur_right_node->right = NULL;
    cur_right_node->parent = par;
    cur_right_node->module = modules + n - 1;
    cur_right_node->cutline = UNDEFINED_CUTLINE;
    par->right = cur_right_node;

    // Return new left node
    return cur_left_node;
  }
  // If function is not yet at end of tree
  else {

    // Allocate memory for new left node
    node_t* cur_left_node = (node_t*)malloc(sizeof(node_t));

    // Set up all values for new left node (no module, vertical cutline, parent = par)
    cur_left_node->module = NULL;
    cur_left_node->cutline = V;
    cur_left_node->parent = par;

    // If passed parameter par is not null...
    if (par != NULL) {

      // Allocate memory for new right node
      node_t* cur_right_node = (node_t*)malloc(sizeof(node_t));
      ...
      // Set up both of parent's child modules
      par->right = cur_right_node;
      par->left = cur_left_node;
    }

    // Recurse through left node , incrementing n by 1
    init_slicing_tree(cur_left_node, n + 1);
