...
*/
{
    ...
    int max = cur_game->cols * cur_game->rows;
    int displace = cur_game->cols * row + col;
    if ( displace < max ) {
 ...
 */
{
    ...
    int index;
    for (index = 0; index < cur_game->rows * cur_game->cols; index++) {
        if ( cur_game->cells[index] == -1) {
            return 1;
        }
        if ( !(index < cur_game->cols) ) {
            if ( cur_game->cells[index] == cur_game->cells[index - cur_game->cols] ) return 1;
        }
        if ( !(index >= cur_game->cols * (cur_game->rows - 1) ) ) {
            if ( cur_game->cells[index] == cur_game->cells[index + cur_game->cols] ) return 1;
        }
        if ( !(index % cur_game->cols == 0 )  ) {
            if ( cur_game->cells[index] == cur_game->cells[index - 1] ) return 1;
        }
        if ( !(index % cur_game->cols == cur_game->cols - 1) ) {
            if ( cur_game->cells[index] == cur_game->cells[index + 1] ) return 1;
        }
    }
